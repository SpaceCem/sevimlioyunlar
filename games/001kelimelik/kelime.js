...
/**
 * STRATEJİK YENİLEME: Sadece kullanılan harfleri yeniler.
 */
function kullanilanHarfleriYenile(hucreListesi) {
    hucreListesi.forEach(tile => {
        const h = harfHavuzu[Math.floor(Math.random() * harfHavuzu.length)];
        
        tile.classList.remove('selected', 'gorev-tile', 'multiplier-2', 'multiplier-3', 'multiplier-5', 'enemy-selected');
        tile.innerHTML = `${h}<span class="tile-point">${harfPuanlari[h] || 1}</span>`;
        tile.dataset.multiplier = "1";
        
        const oldTag = tile.querySelector('.bonus-tag');
...
async function saldir() {
    const kelime = secilenHücreler.map(t => t.textContent[0]).join('').toUpperCase('tr-TR');
    ...
    secilenHücreler.forEach(t => {
        const anaPuan = harfPuanlari[t.textContent[0]] || 1;
...
async function rakipAI() {
    ...
    for (let i = 0; i < Math.min(800, karistirilmis.length); i++) {
        ...
        for (const h of kelime) {
            if (kopya[h] > 0) {
                kopya[h]--;
                const t = tiles.find(tile => tile.textContent[0] === h);
    ...
    if (enIyiKelime) {
        ...
        let tempKelime = enIyiKelime;
        for (let h of tempKelime) {
            const t = tiles.find(tile => tile.textContent[0] === h && !rakipKullandigiTiles.includes(tile));
...
function stageGecis() {
    if (currentStage < 5) {
        ...
        updateUI();
        tabloOlustur(); // Yeni bölümde tablo komple yenilenir
